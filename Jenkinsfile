...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="gloriogo/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/gloriogo/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Staging Deploy') {
			...
			/*
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no daisy@192.168.33.22 docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no daisy@192.168.33.22 docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
                                        -e MYSQL_IP=192.168.33.18 \
                                        -e MYSQL_PORT=3306 \
